...
EncoderOpenGL::EncoderOpenGL():
	EncoderCUDA(),
	m_registeredPBOs(std::unordered_map<unsigned int, RegisteredPBO>()),
	m_registeredTextures(std::unordered_map<unsigned int, RegisteredTexture>())
{
...
std::shared_ptr<NV_ENC_LOCK_BITSTREAM> EncoderOpenGL::EncodeFrame(unsigned int texture, unsigned int target, uint32_t width, uint32_t height, bool iFrame)
{
